...
- **Camera placement is a decision, not a default.** Every engine spawns the camera somewhere arbitrary, and a scene shot from the default camera position looks exactly like nobody decided anything. [[a1-spec|A1]] asks for a deliberately placed camera and a sentence of narration about the choice, because "what does the player see" is the first design question of every scene you will ever build.
